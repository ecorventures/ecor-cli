...
Object.defineProperties(obj,{
  ...
  cfgpath: {
    ...
    writable: false,
    configurable: false,
    value: path.join(process.env.HOME || process.env.APPDATA, '.ecor')
  },
  ...
  getConfig: {
    ...
    configurable: false,
    value: function(){
      var cfg = path.join(this.cfgpath, 'config.json');
      return fs.existsSync(cfg) ? JSON.parse(fs.readFileSync(cfg)) : {};
    }
  },
  ...
  saveConfig: {
    ...
    configurable: false,
    value: function(cfg){
      fs.writeFileSync(path.join(this.cfgpath,'config.json'),JSON.stringify(cfg,null,2));
    }
  },
